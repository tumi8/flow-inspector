...
if __name__ == "__main__":
	...
	live_check_list = device_table.find({'do_live_check': 1, 'status': 1}, {'ip': 1})
	...
	os.rename(tmp_file, args.live_check_list)

	# do the same for snmp_pools
	# read a list of devices from the DB which should be included in the live checks
	snmp_poll_list = device_table.find({'do_snmp': 1, 'status': 1}, {'ip': 1, 'community_string': 1})
	(tmp_fd, tmp_file) = tempfile.mkstemp()
	...
	# move the tmp file to the final location
	os.rename(tmp_file, args.snmp_poll_list)
