...
def findRouteIPTable(ip_src, ip_dst, verbose=False):
	...
	ip_src = IPAddress(ip_src)
	ip_dst = IPAddress(ip_dst)
		...
		for route in collection.find({"type":"eigrp", "ip_src": router, "timestamp": timestamp}).sort("cEigrpRouteMask", -1):
			...
			if (ip_dst in IPNetwork(str(route["ip_dst"]) + "/" + str(route["cEigrpRouteMask"]))):
				...
				if result.count() > 1:
					...
					print "Next Hop: " + router + " to " + str(route["ip_dst"]) + "/" + str(route["cEigrpRouteMask"]) + " is " + route["cEigrpNextHopAddress"] + " (unknown IP, " + route["cEigrpRouteOriginType"] + ")"
				else:
...
def main():
	...
	findRouteIPTable(args.src_ip, args.dst_ip)
	print ""
	print "Using EIGRP route information"
	findRouteEIGRP(args.src_ip, args.dst_ip)
